...
def addSurface(nodeX, nodeY, nodeZ, model_in, sfLocInfo, val_out, normal):
    Nx = len(nodeX) - 1
    Ny = len(nodeY) - 1
    Nz = len(nodeZ) - 1
    ...
    if normal == 'z':
        ...
        zq = griddata((sfLocInfo[:, 0], sfLocInfo[:, 1]), sfLocInfo[:, 2],
                      (xq, yq), method='linear', fill_value=np.mean(sfLocInfo[:, 2]))
    ...
    if normal == 'y':
        ...
        yq = griddata((sfLocInfo[:, 0], sfLocInfo[:, 2]), sfLocInfo[:, 1],
                      (xq, zq), method='linear', fill_value=np.mean(sfLocInfo[:, 1]))
